...
哈希表  | 如果关键字已知，则增查改很快，删除有时也快，比树快         | 删除慢，如果不知道关键字，存取慢，对空间利用不充分
...
* 2-3-4树：等价于红黑树
    * 每个节点可以保存1-3个数据
    * 所有叶节点都在同一层
    * 名字2：有一个数据的节点总是有两个子节点
    * 名字3：有两个数据的节点总是有三个子节点
    * 名字4：有三个数据的节点总是有四个子节点，也就是说2-3-4树可称为四叉树
    * 非叶节点的子节点有一个数据项的节点必须有两个子节点
    * 简而言之，非叶节点的字节点数总是比它的数据项多一
    * 可与红黑树相互转换
* 红黑二叉树：由2-3-4树转换而来；是平衡二叉树的一种，放弃完全平衡，最求大致平衡
    * 假使每个节点分为红黑二色
    * 根总是黑色的
    ...
    * 所以红黑树只是为了保证最大平衡
    * 红黑树插入比2-3-4树慢，但空间利用率比2-3-4树高；2-3-4树实现比红黑树简单

#### 哈希表：根据关键码值(Key value)而直接进行访问的数据结构
    增改查快，删有时快；比树快；
    基于数组，创建后难以扩展；有些哈希表被填满后，性能下降严重；
    所以使用时最好清楚知道数据大小，或者定期转移数据
    且哈希表无法有序遍历数据；
    如果无需有序遍历，且提前预知数据大小，其速度和易用性无与伦比；

#### 堆：实现优先级队列的另一种结构；一种特殊的完全二叉树（和JVM内存中的堆无关）
    数组实现的优先级队列，增加时需要移动大量数据项，所以比较慢；
    堆删除比优先级数组慢，但插入快很多；所以如果有很多插入操作，额可以使用堆来实现优先级队列
...
PS:忽然觉得自己是不配被称为程序员的，无非是一个Java的curd的码农罢了。代码世界中太多的如艺术般精致、伟岸的东西，已经不是我能够触及到的了。我无非只是为了一份糊口的工作，才稍许努力地学习。
